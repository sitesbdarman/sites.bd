...
export function PaymentContent({ invoiceId }: { invoiceId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {submitted && <ThankYouModal invoiceNumber={invoice.invoice_number} orderId={invoice.order_id} transactionId={transactionId} />}
  </div>;
}

function ThankYouModal({ invoiceNumber, orderId, transactionId }: { invoiceNumber: string; orderId: string; transactionId: string }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 px-4 py-8 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="thankyou-title">
      <div className="w-full max-w-lg overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[var(--shadow-float)]">
        <div className="bg-slate-950 px-6 py-9 text-center text-white sm:px-8">
          <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-emerald-400/15 text-2xl ring-1 ring-emerald-300/20">✓</div>
          <p className="mt-4 text-xs font-black uppercase tracking-[.2em] text-emerald-300">Thank you</p>
          <h2 id="thankyou-title" className="mt-2 text-2xl font-black tracking-[-.02em] sm:text-3xl">Payment received!</h2>
          <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-slate-300">Invoice <strong className="text-white">{invoiceNumber}</strong> (transaction {transactionId}) is now with our team for verification. We&apos;ll activate your order shortly.</p>
        </div>
        <div className="p-5 sm:p-6">
          <p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">What&apos;s next</p>
          <div className="mt-3 grid gap-2.5">
            <Link href={`/dashboard/orders`} className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/30"><span><span className="block font-black text-slate-950">Track your order</span><span className="text-xs text-slate-500">Order {orderId.slice(0, 8)} — see approval status</span></span><span className="text-blue-600 transition group-hover:translate-x-0.5">→</span></Link>
            <Link href="/support/knowledge-base" className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/30"><span><span className="block font-black text-slate-950">Setup guides</span><span className="text-xs text-slate-500">DNS, hosting connect & email — step by step</span></span><span className="text-blue-600 transition group-hover:translate-x-0.5">→</span></Link>
            <Link href="/dashboard/tickets" className="group flex items-center justify-between rounded-xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/30"><span><span className="block font-black text-slate-950">Need help?</span><span className="text-xs text-slate-500">Open a support ticket, we reply fast</span></span><span className="text-blue-600 transition group-hover:translate-x-0.5">→</span></Link>
          </div>
          <Link href="/dashboard" className="mt-5 flex min-h-11 w-full items-center justify-center rounded-xl bg-slate-950 px-5 text-sm font-black text-white">Go to dashboard →</Link>
        </div>
      </div>
    </div>
  );
