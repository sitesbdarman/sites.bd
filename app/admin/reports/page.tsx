...
export default async function AdminReportsPage(){
  ...
  const revenue=(paid??[]).reduce((s:any,r:any)=>s+Number(r.total||0),0);
  const pendingPayments=(payments??[]).filter((p:any)=>p.status==='pending_review').length;
  const expiring=(domains??[]).filter((d:any)=>d.expires_at && new Date(d.expires_at).getTime()<=Date.now()+30*86400000 && new Date(d.expires_at).getTime()>=Date.now()).length;
  const recentOrders=(orders??[]).filter((o:any)=>new Date(o.created_at).getTime()>=Date.now()-7*86400000).length;
