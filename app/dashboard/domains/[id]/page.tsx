...
export default async function DomainDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DashboardLayout pageTitle="Domain Details" userEmail={user.email ?? null}>
    ...
    <div className="mb-5 mt-4 flex flex-col gap-3 rounded-[--radius-surface] border border-gray-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="mt-5 grid gap-5 md:grid-cols-3"><div className="rounded-[--radius-surface] border border-gray-200 bg-white p-5"><h3 className="font-bold">Connect Service</h3><p className="mt-2 text-sm text-gray-500">Link hosting or other services to this domain from your service order.</p></div><div className="rounded-[--radius-surface] border border-gray-200 bg-white p-5"><h3 className="font-bold">Guides & Information</h3><p className="mt-2 text-sm text-gray-500">DNS, nameserver, SSL and free-hosting setup guides can be managed from admin content.</p></div><div className="rounded-[--radius-surface] border border-gray-200 bg-white p-5"><h3 className="font-bold">Special Offers</h3><p className="mt-2 text-sm text-gray-500">Promotions and renewal offers will appear here when configured.</p></div></div>
  </DashboardLayout>;
}
