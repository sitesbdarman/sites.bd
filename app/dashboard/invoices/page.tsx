...
export default async function InvoicesPage() {
  ...
  const { data: invoices, error } = user
    ? await supabase.from("invoices").select("invoice_number,status,currency,total,created_at,due_at").eq("customer_id", user.id).order("created_at", { ascending: false })
    : { data: [], error: null };
