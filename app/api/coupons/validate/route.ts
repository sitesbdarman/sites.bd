...
export async function POST(request: Request) {
  ...
  let hostingPrice=0;
  if (body.hosting?.type !== "custom" && body.hosting?.planId) { const p=getHostingPlanById(body.hosting.planId); if (p && p.type===body.hosting.type) hostingPrice=p.price; }
  const addonIds=Array.from(new Set(Array.isArray(body.addonIds)?body.addonIds:[]));
  const addons=addonIds.map((id)=>getAddonById(String(id))).filter(Boolean) as any[];
