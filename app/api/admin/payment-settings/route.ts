...
export async function PATCH(request: Request){
 ...
 const auth = await assertAdminApi("settings:write"); if (auth.response) return auth.response; let b:any; try{b=await request.json();}catch{return NextResponse.json({success:false,error:"Invalid request body."},{status:400});}
