...
export async function POST(request: Request) {
  ...
  const auth = await assertAdminApi("coupons:write"); if (auth.response) return auth.response;
...
export async function PATCH(request: Request) {
  const originError = assertSameOrigin(request); if (originError) return originError;
  const auth = await assertAdminApi("coupons:write"); if (auth.response) return auth.response; let b:any; try { b=await request.json(); } catch { return NextResponse.json({success:false,error:"Invalid request body."},{status:400}); }
...
export async function DELETE(request: Request) {
  const originError = assertSameOrigin(request); if (originError) return originError;
  const auth = await assertAdminApi("coupons:write"); if (auth.response) return auth.response; const id=new URL(request.url).searchParams.get("id"); if(!id) return NextResponse.json({success:false,error:"Coupon id required."},{status:400});
