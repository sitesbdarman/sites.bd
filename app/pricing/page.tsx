      ...
      <section className="px-5 py-16 lg:px-8 lg:py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {(domainPrices ?? []).map((item: any) => <div key={item.id} className="surface p-6 transition-colors hover:border-gray-300"><div className="text-2xl font-extrabold text-blue-600">.{item.tld}</div><div className="mt-3 text-2xl font-extrabold">{item.currency === "BDT" ? `৳${Number(item.registration_price).toLocaleString("en-BD")}` : `${item.currency} ${Number(item.registration_price).toLocaleString()}`}</div><div className="mt-1 text-sm text-gray-500">Registration / year · Renewal {item.currency === "BDT" ? `৳${Number(item.renewal_price).toLocaleString("en-BD")}` : `${item.currency} ${Number(item.renewal_price).toLocaleString()}`}</div><Link href={`/domains/search?q=example.${item.tld}`} className="mt-5 block rounded-xl bg-blue-50 py-3 text-center font-bold text-blue-700 hover:bg-blue-100">Search .{item.tld}</Link></div>)}
          </div>
        </div>
      </section>
